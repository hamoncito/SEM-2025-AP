...
Diagram \ref{fig:diagram_klas} przedstawia pełną architekturę aplikacji, w której warstwa interfejsu użytkownika jest oddzielona od logiki przetwarzania obrazu oraz inferencji modelu uczenia maszynowego.
Głównym punktem wejścia aplikacji jest klasa MainActivity, odpowiedzialna za inicjalizację środowiska Jetpack Compose oraz utworzenie instancji MainViewModel, który zarządza stanem aplikacji.

Warstwa prezentacji została zaprojektowana modularnie.
Komponent ScreenContent odpowiada za ogólny układ aplikacji, natomiast poszczególne ekrany realizują konkretne funkcje interfejsu.
Ekran PredictionScreen prezentuje wyniki klasyfikacji, natomiast AnalysisPanel generuje dynamiczną listę elementów PredictionRow, z których każdy reprezentuje pojedynczy wynik inferencji.
Dodatkowym elementem warstwy prezentacji jest ekran InfoScreen, który pełni funkcję informacyjną.
Zawiera on szczegółowe opisy wszystkich typów zmian skórnych rozpoznawanych przez model1, wraz z przykładowymi ilustracjami.

Logika przetwarzania obrazu oraz komunikacja z modelem TensorFlow Lite została umieszczona w dedykowanej klasie TFLiteHelper.
Odpowiada ona za przygotowanie danych wejściowych, w tym skalowanie obrazu, konwersję do formatu akceptowanego przez interpreter TFLite oraz zastosowanie preprocessingu zgodnego z wybraną architekturą.
Model zwraca wektor prawdopodobieństw, który jest mapowany na typy zmian skórnych zdefiniowane w enumeracji LesionType.
Wyniki klasyfikacji reprezentowane są w postaci listy obiektów PredictionResult, a następnie zaprezentowane w komponencie PredictionScreen.
...
MainViewModel aktualizuje stan imageBitmap, co automatycznie inicjuje wywołanie metody predict() w klasie TFLiteHelper.
Wyniki inferencji są następnie przekazywane do warstwy prezentacji i wyświetlane w komponencie PredictionScreen w postaci listy obiektów PredictionResult.
Pełen przepływ użytkownika wraz z prezentacją interfejsu przedstawiono na diagramiach \ref{fig:diagram_app1} oraz \ref{fig:diagram_app2}.
