...
Rak podstawnokomórkowy (basal cell carcinoma, BCC) jest najczęściej występującym złośliwym nowotworem skóry, szczególnie u osób o jasnej karnacji.
...
Przykładowe obrazy tej klasy przedstawiono na rysunku \ref{fig:bcc_examples}.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\linewidth]{bcc_examples}
	\caption{Klasa "`basal cell carcinoma''}
	\source{Opracowanie własne}
	\label{fig:bcc_examples}
\end{figure}
...
Zmiana może przekształcić się w złośliwego raka kolczystkomórkowego, przez co wymaga obserwacji.
Pojedyncze obszary rogowacenia słonefznego różnią się od siebie i pojawiają się w każdym miejscu przewlekle eksponowanym na słońce, np. na twarzy, uszach i szyi.
Przykładową fotografię rogowacenia słonecznego przedstawiono na rysunku \ref{fig:actinic_keratosis_examples}.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\linewidth]{actinic_keratosis_examples}
	\caption{Klasa "`actinic keratosis''}
	\source{Opracowanie własne}
	\label{fig:actinic_keratosis_examples}
\end{figure}
...
Przykładowy obraz z klasy NEV przedstawiono na rysunku \ref{fig:nevus_examples}.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\linewidth]{nevus_examples}
	\caption{Klasa "`nevus''}
	\source{Opracowanie własne}
	\label{fig:nevus_examples}
\end{figure}
...
Przykładową zmianę należącą do klasy SEK przedstawiono na rysunku \ref{fig:sek_examples}.

\cite{Bourke_2010}
 \ref{fig:sek_examples}.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\linewidth]{sek_examples}
	\caption{Klasa "`seborrheic keratosis''}
	\source{Opracowanie własne}
	\label{fig:sek_examples}
\end{figure}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\linewidth]{scc_examples}
...
Zmianę przedstawiono na rysunku \ref{fig:melanoma_examples}.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\linewidth]{melanoma_examples}
	\caption{Klasa "`melanoma''}
	\source{Opracowanie własne}
	\label{fig:melanoma_examples}
\end{figure}

Łącznie badany zbiór obejmował 2 298 obrazów, co przedstawiono na wykresie słupkowym \ref{fig:classes_bar} oraz wykresie kołowym \ref{fig:classes_pie}.
...
Diagram \ref{fig:diagram_systemu} przedstawia uproszczoną architekturę systemu oraz ścieżkę interakcji użytkownika z aplikacją.
...
Następnie obraz jest przekazany do lokalnie przechowywanego modelu TFLite, który dokonuje inferencji i zwraca wynik klasyfikacji, który jest wyświetlony na interfejsie użytkownika w postaci procentowych prawdopodobieństw przynależności badanej zmiany do każdej z klas.
...
Systemy wykorzystujące sieci neuronowe mogą w przyszłości wspierać lekarzy w procesie diagnostycznym, odciążać służbę zdrowia oraz zwiększać świadomość pacjentów w zakresie wczesnego wykrywania zmian skórnych.
...
Z tych względów opracowany system należy traktować jako narzędzie wspomagające, którego główną rolą jest zwiększanie świadomości użytkownika oraz ułatwianie wczesnej identyfikacji zmian mogących wymagać konsultacji dermatologicznej.
Ostateczna decyzja diagnostyczna powinna zawsze należeć do lekarza, a system może jedynie wspierać proces podejmowania decyzji, nie zastępując profesjonalnej oceny klinicznej.